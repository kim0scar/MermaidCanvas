...
| **Webbappen (web-first)** | https://visuali2e.com (+ www; Pages-URL `visuali2e-viewer.pages.dev`) — W3: RITA med native-utseendet (v2e-shape), iOS-chromet, PWA (hemskärm+offline), AI-chat `/api/chat` (Cloudflare Pages Function → OpenRouter). Kod: `web/` (apps/viewer + packages/domain·canvas·persistence). Deploy-guide: `web/apps/viewer/GÅ-LIVE.md`, AI-guide: `web/apps/viewer/functions/README-AI.md` | Cloudflare Pages, projekt `visuali2e-viewer` + domän-zon `visuali2e.com` (Registrar, auto-förnyas 2027-05), konto kim.lundqvist@gmail.com |
| **AI-chattens hemligheter** | Cloudflare Pages-secrets på projektet `visuali2e-viewer`: `OPENROUTER_API_KEY` + `ACCESS_CODES` (+ valfri `AI_MODEL`, default `anthropic/claude-sonnet-5`). Sätts av Kim i Terminal: `npx wrangler pages secret put <NAMN> --project-name visuali2e-viewer`. Lokalt: `web/apps/viewer/.dev.vars` (gitignorad) | Cloudflare (krypterade secrets) — ALDRIG i git |
