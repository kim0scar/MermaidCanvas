...
- ⏳ W2 — Fas 2 canvas-MVP: rita i webbläsaren — 3 grundformer (cirkel/rektangel/romb) + pil + text-i-form + flytt; domänmodellen är sanning (tldraw bakom adapter); spara/öppna .md (Tier 1 state-JSON förlustfritt); genererad mermaid genom parse-grind. Klart när: adapter-round-trip-test grönt (rita→spara→öppna→exakt samma) + Playwright-smoke iPhone+Mac + Kim ritar på sin iPhone (touch-känsla 🔴).
