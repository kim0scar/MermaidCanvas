...
NU: MB — noll-avvikelse-garantin FÖRST (Kims order). ⏳ Steg F (bevisa+täta+institutionalisera round-trip) PÅGÅR. ✅ Steg 7 (validering), steg 8 del 1 (Skillflöde-meny). KÖ: steg G (form-genomgång+Kims feedback) → steg 8 del 2 → steg 9 (UI-mall). EFTER: steg 0 (städa rot), 1–5 (UX-111–114). ✅ Steg 6. n8n + M4 PAUSADE. MA + M3 klara.
[grundappen MB: ███░░░░░░░] 2/9 + steg F ⏳   ·   💡 Idébanken: 5 fångade, 2 byggda
...
- ⏳ Steg F — Noll-avvikelse-garantin (Kims order, FÖRST): täta round-trip-luckorna (exakt position; 5 former + textjust/listor/waypoints i ren mermaid), tvinga round-trip maskinellt (pre-commit), skrivskyddad facit-bibel + CLAUDE.md regel 3 omskriven (a/b/c + Apple-robust + metodiskt-genom-former), två agent-par bekräftar. Klart: alla former round-trippar bokstavligt noll avvikelse (re-import) + inget försvinner tyst i ren mermaid, grind maskinell, båda paren gröna.
- ☐ Steg G — Metodisk form-genomgång + Kims feedback (efter F): form för form i appen, Kims ändringar följer F-grinden; steg 8 del 2 (fil-glyf + skill-korpus) + steg 9 (UI-mall) byggs inom denna. Klart: Kim gått igenom alla former + godkänt.
- ◐ Steg 8 — Skill-flöde-meny: DEL 1 ✅ (Skillflöde ersätter n8n/Prompt-Process; Subagent/Tool/MCP/Plugin/Skill/Input/Output/Fil MD+Excel; prompt bara på skill-former; Subskill-namn — commit 17d4144). DEL 2 (container-note + skill-korpus + fil-glyf) byggs inom steg G. Klart: menyn ren, allt round-trippar båda vägar, grind+tester gröna, se-appen.
- ☐ Steg 9 — UI-mall: ta bort Mallar-menyn, iPhone 15 Pro + 16 Pro under UI, namn utanpå ramen, bara proportionell resize (handtag nere höger). RIKTAD UTÖKNING (Kims order 2026-06-18): **phoneFrame fungerar som container** — skärmen äger sitt innehåll (import länkar inre noder som barn; routing respekterar hierarkin) så formerna ligger LOGISKT i skärmen, inte bara visuellt ovanpå. (localPos/zIndex/route-styrning = 💡#6, byggs ej nu.) Klart: device+namn round-trippar, skärm-innehåll blir barn, grind+tester gröna, Kims iPhone-bock.
