...
- ⏳ HOTFIX 1.5.2 (AKUT, Mac-frysning — Kim "kan inte ens klicka på ikonen") — `ZoomableCanvasMac.syncViewport()` saknade likhetskoll OCH anropades synkront från `updateNSView` → `@Published` fyrade varje ritning → oändlig SwiftUI-loop, 94% CPU i 23h, frusen Mac-app (kraschar ej → ingen kraschrapport → slank förbi deploy-mac.sh). Fix: spegla iOS-guards (skriv bara vid ändring) + async-defer. Klart när: Mac-appen startar med CPU ~0% + popupen öppnas; `deploy-mac.sh` fångar hängning maskinellt; BÅDA plattformarna på 1.5.2. (Fix 3-5 pausade, väntar Kims iPhone-fynd.)
